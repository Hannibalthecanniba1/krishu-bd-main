...
export const CakeCeremony: React.FC<CakeCeremonyProps> = ({ onComplete }) => {
  ...
  const [stickyNotes] = React.useState<StickyNote[]>([
    { id: 0, text: 'ITS krishuu ka swag💅🏻!!', x: 20, y: 20 },
    { id: 1, text: 'YUM!!', x: window.innerWidth - 140, y: window.innerHeight - 140 },
  ]);
  ...
  return (
    ...
    >
      <div className="text-3xl mb-8 text-pink-600 font-semibold text-center">
        {!isCut ? (
          <span>✨ khoobsurat hatho se candles jalado hahaaha! ✨</span>
        ) : (
          <span>🎉 💗🫶🏻Happyyy Birthdayyy KRISHUU🫶🏻💗!!! 🎉</span>
        )}
      </div>
      ...
      >
        ...
        {easterEggFound && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
          >
            Bina chaar aankh ke dhundliya DAMNNN apka swag!!
          </motion.div>
        )}
